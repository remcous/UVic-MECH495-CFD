...
% 50x25 intervals plots Simple Boundary Treatment
[u_old, u_new, count] = simpleBoundary(initial_mesh_25, Bi, u_inf, epsilon, length);
[M,N]=size(u_new);
title_prefix = 'Simple Boundary Treatment '; folder='Simple'; 
prefix='simple'; suffix=25;
...
set(gcf, 'Position', get(0, 'Screensize'));hold off;
title([title_prefix, num2str(N-1), 'x', num2str(M-1), ' mesh intervals - Distribution at Boundary']);
saveas(gcf,[pwd '\' folder '\' prefix num2str(suffix) '_bound.png']); close(gcf);

% 100x50 intervals plots Simple Boundary Treatment
[u_old, u_new, count] = simpleBoundary(initial_mesh_50, Bi, u_inf, epsilon, length);
[M,N]=size(u_new);
title_prefix = 'Simple Boundary Treatment '; folder='Simple'; 
prefix='simple'; suffix=50;
figure;pcolor(linspace(0,2,N), linspace(0,1,M), u_new);colorbar;
daspect([1 1 1]);set(gcf, 'Position', get(0, 'Screensize'));
title([title_prefix, num2str(N-1), 'x', num2str(M-1), ' mesh intervals at iteration ', num2str(count)]);
saveas(gcf,[pwd '\' folder '\' prefix num2str(suffix) '_new.png']); close(gcf);
...
% 200x100 intervals plots Simple Boundary Treatment
[u_old, u_new, count] = simpleBoundary(initial_mesh_100, Bi, u_inf, epsilon, length);
[M,N]=size(u_new);
title_prefix = 'Simple Boundary Treatment '; folder='Simple'; 
prefix='simple'; suffix=100;
figure;pcolor(linspace(0,2,N), linspace(0,1,M), u_new);colorbar;
daspect([1 1 1]);set(gcf, 'Position', get(0, 'Screensize'));
title([title_prefix, num2str(N-1), 'x', num2str(M-1), ' mesh intervals at iteration ', num2str(count)]);
saveas(gcf,[pwd '\' folder '\' prefix num2str(suffix) '_new.png']); close(gcf);
